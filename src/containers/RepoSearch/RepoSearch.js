...
class RepoSearch extends Component {
    ...
    repositoryMapper = () => {
        ...
        let list = this.props.repo.map(({node}) =>
            ...
            />))
        let moreList = this.props.pageInfo.hasNextPage ? (
                <Button onClick={this.fetchMoreRepo} className={classes.btnList}> More List </Button>) :
        ...
        let formElement = (
            <div className="repoBox">
                ...
                <button
                    ...
                    onClick={this.props.clearSearch}> Clear All Results
                </button>
            </div>)
        let repoList = this.props.repo.length ? this.repositoryMapper() : null;
        let status = (<h4 className="status"> {this.props.status} </h4>)
        let repoCount = this.props.repoCount ? (
            <p className="searchCount"> Search Result: Total {this.props.repoCount} Repository </p>) : null;
        return (
